...
[![Halo](https://img.shields.io/badge/Halo-2.26.0+-blue)](https://halo.run)
...
- Node.js `>=24.18.0 <25`（本地版本文件与 CI 固定为 `24.21.0`）
- pnpm `12.6.0`
- Java 21+ (运行 Halo)

使用 nvm 时，在项目目录执行 `nvm use`，再运行下方的 pnpm 命令；其他版本管理器可读取 `.node-version`。
