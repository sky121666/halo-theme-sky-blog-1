...
function clearCategoryError(host) {
  host?.querySelector?.('[data-category-runtime-error]')?.remove();
}

function showCategoryError(host, message, retry) {
  if (!host?.isConnected) return;
  clearCategoryError(host);

  const alert = document.createElement('div');
  alert.dataset.categoryRuntimeError = 'true';
  alert.className = 'alert alert-warning mx-auto mb-4 max-w-7xl';
...
(function() {
  ...
  Alpine.data('categoriesPage', () => ({
    ...
    init() {
      ...
      const initialNext = this.$root.querySelector('#initial-next-url');
      this.nextUrl = initialNext?.value || '';

      // 从现有 DOM 抽取首屏文章，供侧边栏预览
      const cards = Array.from(this.$root.querySelectorAll('#category-posts article'));
    ...
    destroy() {
      ...
      clearCategoryError(this.$root);
    },

    isActive() {
      return !this._disposed && this.$root?.isConnected && !this._abortController?.signal.aborted;
    },
    ...
    enableInfiniteScroll() {
      const sentinel = this.$root.querySelector('#infinite-sentinel');
      if (!sentinel) return;
    ...
    async switchCategory(slug) {
      if (!this.isActive() || this.isSwitching || this.activeSlug === slug) return;
      const previous = { slug: this.activeSlug, name: this.activeName, nextUrl: this.nextUrl };
      ...
      try {
        ...
        if (e?.name === 'AbortError' || !this.isActive()) return;
        this.activeSlug = previous.slug;
        this.activeName = previous.name;
        this.nextUrl = previous.nextUrl;
        skyDebug.warn('categories', '切换分类失败', e);
        showCategoryError(this.$root, '分类切换失败，已恢复原列表。', () => this.switchCategory(slug));
      } finally {
        if (this.isActive()) {
          ...
          this._switchTimer = setTimeout(() => {
            if (this.isActive()) this.isSwitching = false;
          }, 200);
        }
      }
    },
     ...
     */
    async loadMore() {
      if (!this.isActive() || !this.nextUrl || this.isLoading) return;
      this.isLoading = true;
      try {
        ...
        if (e?.name === 'AbortError' || !this.isActive()) return;
        skyDebug.warn('categories', '加载更多失败', e);
        showCategoryError(this.$root, '更多文章加载失败。', () => this.loadMore());
      } finally {
        if (this.isActive()) this.isLoading = false;
      }
    },
     ...
     */
    findNameBySlug(slug) {
      const btn = this.$root.querySelector(`button[data-slug="${CSS.escape(slug)}"] span.font-medium`);
    ...
    renderList(innerHtml) {
      const container = this.$root.querySelector('#category-posts');
      if (!container) return;
  ...
  Alpine.data('categoryArchive', () => ({
    ...
    async loadMore() {
      ...
      this.isLoading = true;
      try {
        const res = await fetch(this.nextUrl, { signal: this._abortController?.signal });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        ...
        if (e?.name === 'AbortError' || !this.isActive()) return;
        skyDebug.warn('categories', '归档页加载更多失败', e);
        showCategoryError(this.$root, '更多文章加载失败。', () => this.loadMore());
      } finally {
        if (this.isActive()) this.isLoading = false;
      }
