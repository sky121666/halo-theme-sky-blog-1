...
| ------------------------- | ---------------------------------------------------------- | ---------------- | -------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PluginLinks`             | `/links`、详情、RSS 权限/游标、申请降级、评论与首页 Finder | `2.2.1`          | `2.2.1`        | `compatible-tested` | `templates/links.html`、`templates/modules/links/`、`src/apps/links/runtime.js`、`templates/modules/index/content.html`、`templates/modules/widgets/links.html`、`settings.yaml`、`scripts/verify-links-runtime.mjs` |
...
| `seo-tools`               | Head 单一所有权、canonical、社交元数据、结构化数据降级     | `1.9.5`          | -              | `inferred`          | `templates/modules/seo-head.html`、`scripts/verify-seo-contracts.mjs`、`docs/system/adaptation/main-halo-runtime-audit.md`                                                                                           |
| `PluginFeed`              | Head RSS 自动发现链接与 `/feed.xml`                        | `1.5.0`          | `1.5.0`        | `compatible-tested` | `templates/modules/seo-head.html`、`scripts/verify-plugin-pages.mjs`、`docs/system/adaptation/main-halo-runtime-audit.md`                                                                                            |
...
主实例的 45 个安装插件、启动状态、页面结果、矩阵外分类和上游阻塞项记录在[主 Halo 插件运行态审计](./main-halo-runtime-audit.md)。当前 28 个插件有显式主题契约，17 个处于矩阵外；`link-submit` 已退出当前主题集成，保留在 Halo 中的禁用安装不会构成 Links 依赖。运行态审计中的 27/18 是 2026-07-17 的历史快照，不会自动抬升本矩阵的 `testedVersion`。版本脚本会同时打印矩阵外插件清单；矩阵外插件可能无需主题契约，也可能是尚未完成适配的缺口，必须按运行审计分类判断。

`2.2.37` 热加载后的页面、DOM、PJAX 与深度边界结果只记入运行态审计；当前启用表面的基础 smoke 为 `11/11`，深度 smoke 为 `31/31`，另跳过禁用的 Friends 路由。未覆盖真实 UI 或业务流程的插件仍不填写 `testedVersion`：Contact Form 1.6.4 仅完成变量、宿主与资源契约；AI Assistant 2.2.4 的 `testedVersion` 只覆盖已有摘要小部件，不扩展到模型生成、RAG 或编辑器 AI。

## 上游版本差距

上游源码或应用市场基线核对日期：2026-07-28。以下差距只用于安排回归，不会自动改写 `contractVersion` 或 `testedVersion`。

| 插件                      | 当前契约 | 当前上游/市场基线 | 当前处理                                                                                           |
| ------------------------- | -------- | ----------------- | -------------------------------------------------------------------------------------------------- |
| `PluginLinks`             | `2.2.1`  | `2.2.1`           | 已采用 linksTitle 安全回退、LinkVo 状态枚举和公开 linkFeedFinder，并完成独立运行态回归             |
...
| `plugin-friends`          | `1.4.6`  | `1.4.6`           | 上游已归档且当前主实例为禁用态，不参与 PluginLinks 2.2.1 的独立适配结论                            |

## 重点兼容点

| 功能           | 注意事项                                                                                                                                        |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Links 2.2.1    | `linksTitle` 可安全回退；状态枚举必须先 `name()`；公开 RSS 关闭时 Finder 返回空分页，主题不显示动态区；申请入口仅使用主题 URL、评论或邮箱降级   |
...
| Friends 路由异常   | 当前主实例已禁用 Friends；它不参与 PluginLinks 2.2.1 独立适配，历史组合结论见运行态快照 |
