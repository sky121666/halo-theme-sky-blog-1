...
2026-09-24 待发布的主题 `2.2.40` 代码已将 `spec.requires` 提高到 `>=2.26.0`，本地 Halo Pro `2.26.1` 已完成元数据 Reload/READY 检查，2026-09-24 05:36 UTC 核验时站点激活主题 2，门槛调整后的主题 1 页面待复测；依赖升级、准确的测试组合及待完成的供应链时龄门禁见 [依赖适配记录](../development/dependency-upgrade-2026-09-24.md)。下文提到的 `2.2.39` 与 `>=2.23.0` 是当时的历史快照。
