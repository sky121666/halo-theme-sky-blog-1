...
> 快照日期：2026-07-15（含 `2.2.33` 热加载完整复验）
> 目标实例：本地容器 `halo`，Halo Pro `2.25.4`，`http://localhost:8090`
> 当前渲染主题：`theme-sky-blog-1 2.2.33`（live mount + Theme Reload API）

本报告初始审计基于 `2.2.23`，当前结论已合并 `2.2.33` Build、Reload 和页面缓存刷新后的验证。它只记录该实例在快照时刻的安装状态和真实页面证据，不替代[插件适配矩阵](./plugin-adaptation.md)中的主题契约。`通过` 只表示列出的表面已通过，未执行提交、登录、上传、评论等会修改数据的操作。

## 结论

- 主容器 `halo` 正在运行，镜像为 Halo Pro `2.25.4`，主端口为 `8090`；本文所有运行态结论均来自该实例，不使用 `8091` 的备用 Halo。
- 主题 `2.2.33` 已完成 Build，Theme Reload API 返回 200，随后页面缓存刷新接口返回 200；热开发使用 live mount，同一版本内无需重新安装主题。
...
- 深度页面/API 检查通过 `36/36`；这仍只代表脚本覆盖的读取型页面、标记和 API，不等于全部插件业务流程兼容。
...
- Vote、Shiki、Text Diagram 和 Hyperlink Card 已在 `2.2.33` 真实浏览器中渲染；Shiki 1.4.1 的真实样本包含 26 个代码块，已通过两轮 PJAX 归一化、折叠及明暗切换，Text Diagram 与 Hyperlink Card 的既有回归继续通过。
...
- 18 个矩阵外插件中，1 个仍有启用前主题决策，2 个需要通用正文回归，15 个属于认证、编辑器、存储、备份、SEO 或运行平台能力。`PluginContactForm 1.6.4` 与 `ai-assistant 2.2.4` 仍高于主题现有契约且未完成各自完整流程回归，当前不能宣称“全员兼容”。
- 严格版本门禁 `pnpm verify:plugin-versions -- --strict` 当前按预期退出 1，未收口项是上述 2 个高于契约且未测试的安装版本；普通运行态版本检查通过不等于可以抬升它们的 `testedVersion`。
...
| `PluginContactForm`       | `1.6.4`，已启动        | Loader 引用与资源 HTTP 200 已确认，没有表单节点或贴边入口                             | 高于 1.6.3 主题契约；UI、校验、上传和提交仍未验证                                  |
| `ai-assistant`            | `2.2.4`，已启动        | RAG UI 资源请求为 200，公开页面未发现 `<ai-summary-widget>`                           | 资源可用不等于 AI Summary 兼容；2.2.4 总结流程仍无样本                             |
...
`link-submit 1.0.7` 与 PluginLinks 2.0.0 可以同时启动。PluginLinks 2.1.0～2.2.1 会在该组合中触发同类 `ObjectMapper` 类加载冲突；官方仓库已有相关修复 PR，但尚未合并或正式发布，附带测试 JAR 不能作为主题正式兼容基线。主题 2.2.33 已通过按需加载官方资源和运行时降级闭环前台申请入口。
...
4. 为 Contact Form、AI Summary 建立专用测试内容或配置，再执行浏览器交互验收。
5. Contact Form 1.6.4 与 AI Assistant 2.2.4 完成专用样本回归前，不填写 `testedVersion`，也不宣称全部插件兼容。
