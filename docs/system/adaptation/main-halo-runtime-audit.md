...
本页保存此前的历史验收快照，其中 `>=2.23.0` 是当时的主题最低要求；`2.2.40` 已在 `theme.yaml` 调整为 `>=2.26.0`。2026-09-24 05:36 UTC 时主题 1 页面待复测，后续已在 Halo Pro `2.26.1` 上临时激活主题 1 并完成当前代码的页面与插件读取复测，详见 [发布前复测](../development/dependency-upgrade-2026-09-24.md#2026-09-25-发布前复核)。本页历史测试结果不因最低版本改动而自动更新。
