...
2026-09-24 已在待发布的 `2.2.40` 代码中升级前端/构建依赖，并将最低 Halo 要求提高到 `>=2.26.0`；本页以下仍是 **2.2.39 的历史快照**，不能据此声称新代码完成同等浏览器交互验收。依赖升级后、最低 Halo 门槛调整前的 114 个原始 URL 的 HTTP/缓存复测、33/33 插件读取型烟测、浏览器抽查，以及调整后的受限复测与供应链时龄门禁见 [依赖适配记录](../development/dependency-upgrade-2026-09-24.md)。
